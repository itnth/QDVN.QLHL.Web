...
export default defineComponent({
    ...
    setup(props, ctx) {
        ...
        const columns = [
            {
                title: 'Người thực hiện', dataIndex: 'AssignToName', key: 'AssignToName',
            ...
            {
                title: 'Mã kế hoạch', dataIndex: 'TrainingPlanCode', key: 'TrainingPlanCode',
                customFilterDropdown: true,
                onFilter: (value: any, record: any) => {
                    return record.Code.toString().toLowerCase().includes(value.toLowerCase());
                },
        ...
        const confirmDelete_Click = async (record: TrainingPlan) => {
            ...
            const res: BaseRespone = await axios.post('ImplementTrainingPlans/SaveData', Array<TrainingPlan>(record))
            if (res && res.Success) {
                loadData();
            }
        };
        ...
        return {
            ...
            data,
            loadData,
            implementTrainingPlansDetail, moment
        }
    },
})